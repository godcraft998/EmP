...
local GuitarScript = StarterPlayer.Modules.Interface.Loader.Events.JamSessionHandler.GuitarMinigame

-- Packages / modules
local FastSignal = require(ReplicatedStorage.Modules.Packages.FastSignal)
local BeatMapFormatter = require(GuitarScript.BeatMapFormatter)
local NoteHandler = require(GuitarScript.NoteHandler)
local ScoreHandler = require(GuitarScript.ScoreHandler)
local ImageUtils = require(GuitarScript.ImageUtils)
...
local guiRoot = nil
local Assets = GuitarScript.Assets
...
-- Side-effect: thay đổi các ImageButton trong GUI template (GuitarScript.GuitarMinigame.Page.Main.Bottom)
function Events._Init()
    local bottom = GuitarScript.GuitarMinigame.Page.Main.Bottom
...
function Events.Open()
    ...
    guiRoot = GuitarScript.GuitarMinigame:Clone()
...
function Events.PlayChart(songName, difficulty, offsetMs)
    ...
    local bottom = guiRoot.Page.Main.Bottom
    local songModule = require(GuitarScript.Songs[songName])
